...
import { SetupEventsReturn, Torii, build } from '@iroha2/client'
...
function displayStatus(status: model.PipelineStatus): string {
...
async function startListening() {
  currentListener.value = await Torii.listenForEvents(toriiPre, {
    filter: build.filter.pipeline({ entityKind: 'Transaction', statusKind: 'Committed' }),
  })
