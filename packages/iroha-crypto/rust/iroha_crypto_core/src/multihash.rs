...
impl FromStr for DigestFunction {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match s {
            ...
            _ => Err("The specified digest function is not supported."
                .to_owned()
                .into()),
        }
    }
...
impl TryFrom<Vec<u8>> for Multihash {
    ...
    fn try_from(bytes: Vec<u8>) -> Result<Self> {
        ...
        let digest_size = bytes
            .next()
            .ok_or_else(|| "Failed to parse digest size.".to_owned())?;
        ...
        if payload.len() == digest_size as usize {
            ...
            Err("The digest size is not equal to the actual length."
                .to_owned()
                .into())
        }
    }
    ...
    fn try_from(multihash: &Multihash) -> Result<Self> {
        ...
        bytes.push(
            multihash
                ...
                .len()
                .try_into()
                .map_err(|err| format!("failed to parse payload len: {}", err))?,
        );
