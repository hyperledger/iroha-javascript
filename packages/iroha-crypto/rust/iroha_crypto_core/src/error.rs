...
pub struct Error {
    inner: Box<dyn ToString + Send + Sync + 'static>,
}
...
impl Display for Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        Display::fmt(&self.inner.to_string(), formatter)
    }
}

impl Debug for Error {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        Debug::fmt(&self.inner.to_string(), formatter)
    }
}

impl AsRef<dyn ToString + Send + Sync + 'static> for Error {
    fn as_ref(&self) -> &(dyn ToString + Send + Sync + 'static) {
        self.inner.as_ref()
    }
}

impl AsRef<dyn ToString + 'static> for Error {
    fn as_ref(&self) -> &(dyn ToString + 'static) {
        self.inner.as_ref()
    }
}

impl Deref for Error {
    type Target = dyn ToString + Send + Sync + 'static;
...
// impl<E: ToString + Send + Sync + 'static> From<E> for Error {
...
impl Error {
    ...
    pub fn new(error: impl ToString + Send + Sync + 'static) -> Self {
