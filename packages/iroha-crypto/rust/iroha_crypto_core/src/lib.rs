...
impl FromStr for Algorithm {
    ...
    fn from_str(algorithm: &str) -> Result<Self> {
        ...
        match algorithm {
            ...
            x => Err(format!("The {} algorithm is not supported.", x).into()),
        }
    }
}
...
impl TryFrom<&PublicKey> for Multihash {
    ...
    fn try_from(public_key: &PublicKey) -> Result<Self> {
        ...
        match public_key.digest_function.as_ref() {
            ...
            _ => Err(format!("Digest function not implemented.").into()),
        }
...
impl Signature {
    ...
    pub fn verify(&self, message: &[u8]) -> Result<()> {
        ...
        match result {
            Ok(true) => Ok(()),
            _ => Err(format!("Signature did not pass verification").into()),
        }
    }
}
