...
import { getTypeDef, paramsNotation, TypeDefInfo, TypeDef } from '@iroha/scale-codec-legacy';
...
export function formatType(
    ...
    setImports(imports, [typeDef.type]);
    ...
    switch (typeDef.info) {
        ...
        case TypeDefInfo.DoNotConstruct: {
            setImports(imports, ['DoNotConstruct']);
        ...
        case TypeDefInfo.Tuple: {
            ...
            return formatTuple((typeDef.sub as TypeDef[]).map((sub) => formatType(sub.type, imports)));
        }
        ...
        case TypeDefInfo.VecFixed: {
            ...
            setImports(imports, ['Vec']);
            return formatVec(formatType(type, imports));
        }

        case TypeDefInfo.BTreeMap: {
            setImports(imports, ['BTreeMap']);

            const [keyDef, valDef] = typeDef.sub as TypeDef[];

            return formatBTreeMap(formatType(keyDef.type, imports), formatType(valDef.type, imports));
        }

        case TypeDefInfo.BTreeSet: {
            setImports(imports, ['BTreeSet']);

            const valDef = typeDef.sub as TypeDef;

            return formatBTreeSet(formatType(valDef.type, imports));
        }

        case TypeDefInfo.HashMap: {
            setImports(imports, ['HashMap']);

            const [keyDef, valDef] = typeDef.sub as TypeDef[];

            return formatHashMap(formatType(keyDef.type, imports), formatType(valDef.type, imports));
        }

        case TypeDefInfo.Linkage: {
            const type = (typeDef.sub as TypeDef).type;

            setImports(imports, ['Linkage']);

            return formatLinkage(formatType(type, imports));
        }
        ...
        case TypeDefInfo.Result: {
            ...
            return formatResult(formatType(okDef.type, imports), formatType(errorDef.type, imports));
        }
