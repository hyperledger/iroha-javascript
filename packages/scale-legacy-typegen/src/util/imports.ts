...
import { getTypeDef, TypeDefInfo, DEFAULT_CODECS } from '@iroha/scale-codec-legacy';
...
export function setImports(imports: TypeImports, types: string[]): void {
    ...
    types.forEach((type): void => {
        if (ignoredTypes.includes(type)) {
        ...
        } else if ((DEFAULT_CODECS as Record<string, unknown>)[type]) {
            defaultCodecs[type] = true;
            ...
            if (Array.isArray(typeDef.sub)) {
                typeDef.sub.forEach((subType) => setImports(imports, [subType.type]));
...
export function createImports(
    ...
    //                 console.warn(
    //                     `\t\tWARN: Overwriting duplicated type '${type}' ${typeToModule[type]} -> ${fullName}`,
    //                 );
    //             }
    ...
    return {
        ...
        // localTypes: Object.keys(definitions).reduce((local: Record<string, TypeExist>, mod): Record<
        //     string,
        //     TypeExist
        // > => {
        //     local[mod] = {};
