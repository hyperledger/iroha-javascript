...
function isRustEnum(details: Record<string, string> | Record<string, number>): details is Record<string, string> {
    ...
    if (values.some((v) => isNumber(v))) {
        assert(
            values.every((v) => isNumber(v) && v >= 0 && v <= 255),
...
//  { _enum: { A: 1, B: 2 } }
function _decodeEnum(
    value: TypeDef,
    details: string[] | Record<string, string> | Record<string, number>,
    count: number,
): TypeDef {
    ...
    if (Array.isArray(details)) {
        ...
        value.sub = Object.entries(details).map(
            ([name, type], index): TypeDef => ({
...
function _decodeStruct(value: TypeDef, type: string, _: string, count: number): TypeDef {
    ...
    value.sub = keys
        .filter((name) => !['_alias'].includes(name))
        .map(
            (name): TypeDef =>
                // eslint-disable-next-line @typescript-eslint/no-use-before-define
...
const nestedExtraction: [
    ...
    string,
    TypeDefInfo,
    (value: TypeDef, type: string, subType: string, count: number) => TypeDef,
][] = [
