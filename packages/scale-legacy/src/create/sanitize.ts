...
export function alias(src: string, dest: string, withChecks = true): Mapper {
    return (value: string): string =>
        value.replace(
            ...
            (src): string => (withChecks && BOX_PRECEDING.includes(src[0]) ? `${src[0]}${dest}` : dest),
        );
}
...
export function flattenSingleTuple(): Mapper {
    return (value: string) => value.replace(/\(([^,]+)\)/, '$1');
}
        ...
        for (let index = 0; index < value.length; index++) {
            if (value[index] === '<') {
                ...
                const box = ALLOWED_BOXES.find((box): boolean => {
                    ...
                    return (
                        start >= 0 &&
                        value.substr(start, box.length) === box &&
...
export function removePairOf(): Mapper {
    ...
    return (value: string) => replaceTagWith(value, 'PairOf<', replacer);
}
...
export function removeWrap(check: string): Mapper {
    ...
    return (value: string) => replaceTagWith(value, check, replacer);
}

// eslint-disable-next-line @typescript-eslint/ban-types
export function sanitize(value: String | string, options?: SanitizeOptions): string {
    return mappings.reduce<string>((result, fn) => fn(result, options), value.toString()).trim();
}
