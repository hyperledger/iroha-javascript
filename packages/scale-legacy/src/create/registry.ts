...
export class TypeRegistry implements Registry {
  ...
  public get<T extends Codec = Codec>(
    name: string,
    withUnknown?: boolean
  ...
  public getClassName(clazz: Constructor): string | undefined {
    const entry = [...this.#classes.entries()].find(
      ([, test]) => test === clazz
    );
  ...
  public getOrThrow<T extends Codec = Codec>(
    name: string,
    msg?: string
  ): Constructor<T> {
  ...
  public hasType(name: string): boolean {
    return (
      !this.#unknownTypes.get(name) &&
      (this.hasClass(name) || this.hasDef(name))
    );
  }
  ...
  // eslint-disable-next-line no-dupe-class-members
  public register(
    arg1: string | Constructor | RegistryTypes,
    arg2?: Constructor
  ): void {
    ...
    if (isFunction(arg1)) {
      ...
      assert(
        isFunction(arg2),
        () => `Expected class definition passed to '${arg1}' registration`
      );
      assert(
        arg1 !== arg2.toString(),
        () => `Unable to register circular ${arg1} === ${arg1}`
      );
  ...
  private _registerObject(obj: RegistryTypes): void {
    Object.entries(obj).forEach(([name, type]): void => {
      if (isFunction(type)) {
        ...
        assert(
          name !== def,
          () => `Unable to register circular ${name} === ${def}`
        );
