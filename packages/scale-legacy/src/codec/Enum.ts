...
export type EnumConstructor<T = Codec> = new (registry: Registry, value?: any, index?: number) => T;
...
function isRustEnum(
    def: Record<string, keyof InterfaceTypes | Constructor> | Record<string, number>,
): def is Record<string, keyof InterfaceTypes | Constructor> {
    const defValues = Object.values(def);

    if (defValues.some((v) => isNumber(v))) {
        assert(
            defValues.every((v) => isNumber(v) && v >= 0 && v <= 255),
...
function extractDef(
    registry: Registry,
    _def: Record<string, keyof InterfaceTypes | Constructor> | Record<string, number> | string[],
): { def: TypesDef; isBasic: boolean; isIndexed: boolean } {
    ...
    if (isRustEnum(_def)) {
        def = Object.entries(mapToTypeMap(registry, _def)).reduce((def: TypesDef, [key, Type], index): TypesDef => {
            def[key] = { Type, index };
        ...
        def = Object.entries(_def).reduce((def: TypesDef, [key, index]): TypesDef => {
            def[key] = { Type: Null, index };
...
export class Enum implements Codec {
    ...
    constructor(
        registry: Registry,
        def: Record<string, keyof InterfaceTypes | Constructor> | Record<string, number> | string[],
        value?: unknown,
        index?: number,
    ) {
    ...
    public static with(
        Types: Record<string, keyof InterfaceTypes | Constructor> | Record<string, number> | string[],
    ): EnumConstructor<Enum> {
    ...
    public toHuman(isExtended?: boolean): AnyJson {
        return this.#isBasic ? this.type : { [this.type]: this.#raw.toHuman(isExtended) };
    }
    ...
    protected _toRawStruct(): string[] | Record<string, string | number> {
        ...
        const typeMap = Object.entries(this.#def).reduce((out: Record<string, Constructor>, [key, { Type }]) => {
            out[key] = Type;
