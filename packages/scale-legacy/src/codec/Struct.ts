...
/** @internal */
function decodeStructFromObject<T>(
    registry: Registry,
    Types: ConstructorDef,
    value: any,
    jsonMap: Map<any, string>,
): T {
    let jsonObj: Record<string, any> | undefined;
    const inputKeys = Object.keys(Types);

    assert(
        !Array.isArray(value) || value.length === inputKeys.length,
        () => `Struct: Unable to map ${stringify(value)} array to object with known keys ${inputKeys.join(', ')}`,
    );
    ...
    return inputKeys.reduce((raw, key, index): T => {
        ...
        try {
            if (Array.isArray(value)) {
                ...
                (raw as any)[key] =
                    value[index] instanceof Types[key] ? value[index] : new Types[key](registry, value[index]);
            } else if (value instanceof Map) {
                // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
                const mapped = value.get(jsonKey);

                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
                (raw as any)[key] = mapped instanceof Types[key] ? mapped : new Types[key](registry, mapped);
                ...
                if (isUndefined(assign)) {
                    if (isUndefined(jsonObj)) {
                        jsonObj = Object.entries(value).reduce((all: Record<string, any>, [key, value]): Record<
                            string,
                            any
                        > => {
                            // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
                ...
                // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-unsafe-member-access
                (raw as any)[key] = assign instanceof Types[key] ? assign : new Types[key](registry, assign);
            } else {
                throw new Error(
                    `Cannot decode value ${stringify(
                        value,
                    )} (typeof ${typeof value}), expected an input object with known keys`,
                );
            }
        } catch (error) {
    ...
    implements Codec
{
    ...
    constructor(
        ...
        super(
            Object.entries(decodeStruct(registry, mapToTypeMap(registry, Types), value, jsonMap)) as [keyof S, Codec][],
        );
     ...
     */
    public get Type(): E {
        return (Object.entries(this.#Types) as [keyof S, Constructor][]).reduce((result: E, [key, Type]): E => {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
    ...
    public toU8a(isBare?: BareOpts): Uint8Array {
        ...
        return u8aConcat(
            ...entries
                ...
                .map(([key, value]): Uint8Array => value.toU8a(!isBare || isBoolean(isBare) ? isBare : isBare[key])),
        );
    }
}
