...
/** @internal */
function decodeSetFromU8a<V extends Codec = Codec>(
    registry: Registry,
...
/** @internal */
function decodeSetFromSet<V extends Codec = Codec>(
    registry: Registry,
    ValClass: Constructor<V>,
    value: Set<any> | string[],
): Set<V> {
    const output = new Set<V>();
 ...
 * @internal
 */
function decodeSet<V extends Codec = Codec>(
    registry: Registry,
    valType: Constructor<V> | keyof InterfaceTypes,
    value?: Uint8Array | string | string[] | Set<any>,
): Set<V> {
    if (!value) {
        return new Set<V>();
    }
