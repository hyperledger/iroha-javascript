...
/** @internal */
function decodeMapFromU8a<K extends Codec = Codec, V extends Codec = Codec>(
    registry: Registry,
    KeyClass: Constructor<K>,
    ValClass: Constructor<V>,
    u8a: Uint8Array,
): Map<K, V> {
...
/** @internal */
function decodeMapFromMap<K extends Codec = Codec, V extends Codec = Codec>(
    registry: Registry,
    KeyClass: Constructor<K>,
    ValClass: Constructor<V>,
    value: Map<any, any>,
    ...
    value.forEach((val: any, key: any) => {
        try {
            output.set(
                key instanceof KeyClass ? key : new KeyClass(registry, key),
 ...
 * @internal
 */
function decodeMap<K extends Codec = Codec, V extends Codec = Codec>(
    registry: Registry,
    keyType: Constructor<K> | keyof InterfaceTypes,
    valType: Constructor<V> | keyof InterfaceTypes,
    value?: Uint8Array | string | Map<any, any>,
): Map<K, V> {
...
export class CodecMap<K extends Codec = Codec, V extends Codec = Codec> extends Map<K, V> implements Codec {
    ...
    public toRawType(): string {
        return `${this.#type}<${
            this.registry.getClassName(this.#KeyClass) || new this.#KeyClass(this.registry).toRawType()
        },${this.registry.getClassName(this.#ValClass) || new this.#ValClass(this.registry).toRawType()}>`;
    }
