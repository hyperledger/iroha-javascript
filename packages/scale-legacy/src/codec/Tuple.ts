...
type TupleTypes =
    | (Constructor | keyof InterfaceTypes)[]
    | {
...
function decodeTuple(registry: Registry, _Types: TupleConstructors, value?: AnyTuple): Codec[] {
    ...
    const Types: Constructor[] = Array.isArray(_Types) ? _Types : Object.values(_Types);
...
export class Tuple extends AbstractArray<Codec> {
    ...
    public toRawType(): string {
        const types = (Array.isArray(this._Types) ? this._Types : Object.values(this._Types)).map(
            (Type) => this.registry.getClassName(Type) || new Type(this.registry).toRawType(),
        );
    ...
    public toU8a(isBare?: boolean): Uint8Array {
        return u8aConcat(...this.map((entry): Uint8Array => entry.toU8a(isBare)));
    }
}
