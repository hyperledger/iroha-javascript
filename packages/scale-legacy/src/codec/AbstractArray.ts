...
export abstract class AbstractArray<T extends Codec> extends Array<T> implements Codec {
    ...
    public toHuman(isExtended?: boolean): AnyJson {
        return this.map((entry): AnyJson => entry.toHuman(isExtended));
    }
    ...
    public toJSON(): AnyJson {
        return this.map((entry): AnyJson => entry.toJSON());
    }
    ...
    public toString(): string {
        ...
        const data = this.map((entry): string => entry.toString());
    ...
    public toU8a(isBare?: boolean): Uint8Array {
        const encoded = this.map((entry): Uint8Array => entry.toU8a(isBare));
