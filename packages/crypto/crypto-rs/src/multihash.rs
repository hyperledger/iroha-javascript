...
const TS_DIGEST_FUNCTION: &str = r#"
...
#[wasm_bindgen]
extern "C" {
    #[wasm_bindgen(typescript_type = "DigestFunction")]
    pub type DigestFunctionJsStr;
}

impl TryFrom<DigestFunctionJsStr> for DigestFunction {
    type Error = JsError;

    fn try_from(value: DigestFunctionJsStr) -> Result<Self, Self::Error> {
        let digest = Self::from_str(
            &value
                .as_string()
                .ok_or_else(|| JsError::new("Value should be a string"))?,
        )
...
impl From<DigestFunction> for DigestFunctionJsStr {
    fn from(value: DigestFunction) -> Self {
        Self {
...
pub struct Multihash {
    /// digest
    pub(crate) digest_function: DigestFunction,
    /// hash payload
